...
public static class TextEditorModelHelper
{
    ...
    {
        if (rowIndex > model.Content.RowEndingList.Count - 1)
            rowIndex = model.Content.RowEndingList.Count - 1;

        if (rowIndex > 0)
            return model.Content.RowEndingList[rowIndex - 1];
    ...
    {
        if (!model.Content.RowEndingList.Any())
            return 0;
        ...
        var endOfRowTupleExclusive = model.Content.RowEndingList[rowIndex];
    ...
    {
        var rowCountAvailable = model.Content.RowEndingList.Count - startingRowIndex;
        ...
        for (var i = startingRowIndex; i < endingRowIndexExclusive; i++)
        {
            ...
            var endOfRowExclusive = model.Content.RowEndingList[i].EndPositionIndexExclusive;

            var row = model.Content.AllRichCharacters
        ...
        this ITextEditorModel model, int rowIndex, int columnIndex)
    {
        ...
        var tabs = model.Content.TabList
    ...
    {
        var localContent = model.Content;
        ...
        foreach (var textEditorTextSpan in textEditorTextSpans)
        {
            ...
            {
                if (i >= localContent.AllRichCharacters.Count)
                    continue;

                localContent.AllRichCharacters[i].DecorationByte = textEditorTextSpan.DecorationByte;

                positionsPainted.Add(i);
            }
        }

        for (var i = 0; i < localContent.AllRichCharacters.Count - 1; i++)
        {
            if (!positionsPainted.Contains(i))
            {
                // DecorationByte of 0 is to be 'None'
                localContent.AllRichCharacters[i].DecorationByte = 0;
            }
        }
    }
    ...
    {
        return new string(model.Content.AllRichCharacters.Select(rc => rc.Value).ToArray());
    }
    ...
    public static char GetCharacter(
        ...
        int positionIndex)
    {
        if (positionIndex < 0 || positionIndex >= model.Content.AllRichCharacters.Count)
            return ParserFacts.END_OF_FILE;

        return model.Content.AllRichCharacters[positionIndex].Value;
    }
    ...
    public static string GetString(
        ...
        int count)
    {
        return new string(model.Content.AllRichCharacters
    ...
    {
        if (startingRowIndex > model.Content.RowCount - 1)
            return string.Empty;
        ...
        if (lastRowIndexExclusive > model.Content.RowCount - 1)
        {
            endingPositionIndexExclusive = model.Content.DocumentLength;
        }
        else
        {
    ...
    public static RowInformation GetRowInformationFromPositionIndex(
        ...
        int positionIndex)
    {
        for (var i = model.Content.RowEndingList.Count - 1; i >= 0; i--)
        {
            var rowEndingTuple = model.Content.RowEndingList[i];
            ...
            if (positionIndex >= rowEndingTuple.EndPositionIndexExclusive)
            {
                return new(
                    ...
                    i == model.Content.RowEndingList.Count - 1
                        ? rowEndingTuple
                        : model.Content.RowEndingList[i + 1]);
            }
        }

        return new(0, 0, model.Content.RowEndingList[0]);
    }
        ...
        this ITextEditorModel model, int rowIndex, int columnIndex, bool moveBackwards)
    {
        ...
        var lastPositionIndexOnRow = model.Content.RowEndingList[rowIndex].EndPositionIndexExclusive - 1;
        ...
        var startingCharacterKind = model.Content.AllRichCharacters[positionIndex].GetCharacterKind();

        while (true)
        {
            if (positionIndex >= model.Content.AllRichCharacters.Count ||
            ...
            var currentCharacterKind = model.Content.AllRichCharacters[positionIndex].GetCharacterKind();
    ...
    {
        return model.Content.AllRichCharacters.ToImmutableArray();
    }
    ...
    public static CharacterKind GetCharacterKind(this ITextEditorModel model, int positionIndex)
    {
        try
        {
            return model.Content.AllRichCharacters[positionIndex].GetCharacterKind();
        }
    ...
    public static string GetLine(
        this ITextEditorModel model, int rowIndex)
    {
        if (rowIndex < 0 || rowIndex > model.Content.RowCount - 1)
            return string.Empty;
