...
public partial class TextEditorModelModifierTests
{
    ...
    {
        // TODO: Keep this test while laying out all the test cases, because for a NOT empty editor model,...
        //       ...this test is applicable. Then once all the tests are written up delete this specific one.
        throw new NotImplementedException("Test is not applicable given an empty text editor model to start with.");
    ...
    /// <summary>
    /// <see cref="TextEditorModelModifier.Insert(string, CursorModifierBagTextEditor, CancellationToken)"/><br/>
    /// Case: Insert into a not-empty editor (2024-04-14)<br/>
    /// ∙∙∙ Case A:<br/>
    ...
    /// ∙∙∙∙∙∙∙ Idea being that the insertion positionIndex is out of bounds due to being large.<br/>
    /// ∙∙∙ Case E:<br/>
    ...
    public void Insert_NotEmptyEditor()
    {
        ...
        {
            inModel = new TextEditorModel(
                new ResourceUri($"/{nameof(Insert_NotEmptyEditor)}.txt"),
                DateTime.UtcNow,
                ExtensionNoPeriodFacts.TXT,
                (
                    "\n" +   // LineFeed
        ...
        TextEditorModel outModel;
        {
            // What can go wrong when the Insert(...) method?
            //     -Insert the null terminating character "\0"
            // What is expected from the Insert(...) method?
            //     -When inserting, all text that comes after the insertion point
            //         gets shifted by the length of the text inserted.
            //     -As such, one must update all the tracked metadata
            //         such as where the line ending positions are.
            // The SetContent(...) tests broke down into:
            //         -DecreaseCounts
            //         -MaintainCounts
            //         -IncreaseCounts
            //     Is a similar scenario found for Insert(...)?
            //         Well, one cannot decrease counts by way of insertion.
            //         Nor can one maintain counts by way of insertion.
            //         Only IncreaseCounts is applicable here.
