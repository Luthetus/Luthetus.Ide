...
public partial class PartitionTests
{
	/// <summary>
	/// To recreate the bug:
	/// 	Short description: delete the body of the class definition by seleting it, and hitting enter.
	/// 	Long description:
	///     	- Locate the open curly brace, of the class definition.
	///     	- Place cursor at the EndExclusiveIndex of the curly brace character (immediately after it,
	///           	and the cursor is still touching the curly brace, but after it.
	///         - Select the text from the previously described position to the end curly brace of the class definition.
	///         - Note: for the end curly brace you are placing the cursor immediately BEFORE it.
	///           	This will then select all the text within the class definition's body if a deletion occurred.
	///         - "Press" the 'Enter' key.
	///           	This should delete all of the text in the class definition's body,
	///               and then insert a new line.
	///         - But, the result is wrong, a lot of text is deleted yes, but there's seemingly "random" gibberish
	///           	where the text used to be.
	///               The hundreds of lines of text that was deleted is reduced down to about 4 lines, but
	///               again, its just gibberish.
	/// </summary>
	public const string SAMPLE_CASE_THAT_HAS_LINE_ENDINGS_BREAK_BUG = @"using System.Collections.Immutable;
