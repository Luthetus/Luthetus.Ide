...
Execution Terminal"));
	...
	private Task ParseSolution(Key<DotNetSolutionModel> dotNetSolutionModelKey)
	{
		...
		_ = Task.Run(async () =>
		{
			...
			var progressThrottle = new ThrottleOptimized<(ParseSolutionStageKind StageKind, double? Progress, string? MessageOne, string? MessageTwo)>(TimeSpan.FromMilliseconds(250), (tuple, _) =>
			{
