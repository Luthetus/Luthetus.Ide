...
public partial class TextEditorModelModifier : IModelTextEditor
{
        ...
        DeleteKind deleteKind = DeleteKind.Delete)
    {
        ...
        for (var cursorIndex = cursorModifierBag.List.Count - 1; cursorIndex >= 0; cursorIndex--)
        {
            ...
            // Whats with DeleteKind.Delete and DeleteKind.Backspace, am I right? can't live with em, can't live without em.
            {
                // Depending on if one passes the enum: DeleteKind.Delete, or DeleteKind.Backspace,
                // the starting positionIndex for the delete operation might change.
                //
                // The 'DeleteKind.Delete' enum-member is a 'forwards' removal.
                //
                // And the 'DeleteKind.Backspace' enum-member is a 'backwards' removal.
                //
                // To write the code such that a 'forwards'-loop or a 'reverse'-loop
                // is used depending on the enum provided, is a pain.
                //
                // Preferably, the programmer can pick a preference, either the code uses a
                // 'forwards'-loop or a 'reverse'-loop REGARDLESS of the enum provided.
                //
                // So, if I say to myself, "I want to implement 'Delete(...)' as a 'forwards'-loop".
                // Well, is it possible to convert parameters that come alongside a 'DeleteKind.Backspace' enum-member,
                // to parameters would be passed if it were a 'DeleteKind.Delete'.
                //
                // Given that collections have a method, 'RemoveRange(int index, int count)',
                // then I want to use a 'forwards'-loop.
                //
                // The implementation of a collection's 'RemoveRange(int index, int count)'
                // is not necessarily a 'fowards'-loop, but the parameters are
                // accepted in the form of a 'forwards'-loop.
                //
                // Initially when removing from a collection a 'reverse'-loop solution tends to
                // pop into my head, so I don't clobber the indices that im iterating over.
                //
                // But, with the text editor's 'Delete(...)', it seems fine to use a 'forwards'-loop,
                // given that the implementation of 'Delete(...)' separates out 'MetadataCalculations'
                // from the ImmutableArray's 'RemoveRange(...)' logic.
                //
                // That is, the 'MetadataCalculations' is where a loop is needed because any removal
                // is done via the underlying ImmutableArray type.
                //
                // Since, 'MetadataCalculations' does not remove anything while looping,
                // then a 'forwards'-loop feels sensible just so the implementation matches
                // the parameters (since we are given the choice).
                //
                // Furthermore, there is a different between the TextEditorModel and the TextEditorPartition.
                // The Model will view "\r\n" as a single 'character'.
                // Where as the Partition will view "\r\n" as two 'char' values.
                //
                // Therefore, given the string "\r\n",
                // The Model would set a 'count' of the string to be '1'.
                // Where as the Partition would set a 'count' of the string to be '2'.
                //
                // This comes into play with the 'Delete(...)' logic.
                // Initially 'Delete(...)' accepts a count based on 'characters'
                // The TextEditor then converts this 'characters' count into
                // a count of how many 'char-values' made up those 'characters'.
                // Because, some characters require two 'char-values' to be represented.
                //
                // Then, the count of 'char-values' is passed to the Partition's
                // ImmutableList for the 'RemoveRange' method.

                // Converting DeleteKind.Backspace to DeleteKind.Delete is a pain.
                {
                    // Due to the multi-char-value character shenanigans,
                    // one cannot simply take the 'positionIndex' and subtract 'characterCount',
                    // to convert from Backspace to Delete.
                    // 
                    // (i.e. this is incorrect:) deletePositionIndex = backspacePositionIndex - characterCount
                    //
                    // So, what is the simplest multi-char-value string that I can form,
                    // then think through a Backspace operation on it?
                    //
                    // Given: multiCharValueString = "\r\n";
                    //
                    // model.Delete(charValuePositionIndex: 2, characterCount: 1, deleteKind: DeleteKind.Backspace);
                    //
                    // This method invocation means:
                    // "\r\n"
                    //      ^----Start here
                    //    ^---Remove previous character
                    //
                    // var removedCharacter = '\n';
                    // if (removedCharacter is LineEnd):
                    //     var lineEnd = LineEndList[...];
                    //     if (lineEnd is CarriageReturnLineFeed):
                    //         global extraCharValues += 1
                    //
                    // # Include 'extraCharValues' in the expression:
                    // deletePositionIndex = backspacePositionIndex - characterCount - extraCharValues
                    // deleteCount = characterCount + extraCharValues
                    //
                    // Partition.RemoveRange(deletePositionIndex, deleteCount);
                    // 
                    // With the values substituted for their variable names:
                    // Partition.RemoveRange(0, 2);
                    //
                    // Partition:
                    // [
                    //     '\r'
                    ...
                    //
                    // -------------------------------------------------------------------------------
                    // Its because the text editor knows the char-value-position-index from the get-go
                    //
                    // Okay, it makes sense.
                    // I'm going to have a 'forwards'-loop and a 'backwards'-loop.
                    //
                    // The reason for this is:
                    // I cannot know how far backwards to offset a backspace's positionIndex,
                    // to make it a delete's positionIndex, until having iterated over all the
                    // characters backwards.
                    //
                    // So, the Model does either a 'forwards'-loop or a 'backwards'-loop,
                    // depending on the Enum member provided.
                    //
                    // During that loop though, the model needs determine the parameters,
                    // that will be passed to the Partition for 'RemoveRange(int index, int count)'.
                    //
                    // During the loop, if a text editor loops backwards, and sees a multi-char-value
                    // character then:
                    //     -Decrease the charValuePositionIndex relative to the length of the multi-char-value character
                    //     -Increase the charValueCount relative to the length of the multi-char-value character
                    //
                    // In regards to looping forwards, and one sees a multi-char-value character then:
                    //     -Increase the charValueCount relative to the length of the multi-char-value character
                    //
                    // When done, return to the caller the (charValuePositionIndex, charValueCount)
                    //
                    // TODO: What if the text editor model receives an invocation that is between
                    // the char values of a multi-char-value character?
                    // That is, "\r\n" is the text, but we are given a starting positionIndex of '1',
            ...
            var charValueCount = DeleteMetadata(characterCount, initialCursorPositionIndex, cursorModifier, cancellationToken, deleteKind);

            // Now the text still needs to be deleted.
            // The cursorModifier is invalid, because the metadata step moved its position.
            // So, use the 'cursorPositionIndex' variable that was calculated prior to the metadata step.
            DeleteValue(characterCount, initialCursorPositionIndex, cancellationToken);
        }
    }
