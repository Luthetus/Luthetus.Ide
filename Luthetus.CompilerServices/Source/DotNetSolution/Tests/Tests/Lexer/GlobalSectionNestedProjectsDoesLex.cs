...
public class GlobalSectionNestedProjectsDoesLex
{
    [Fact]
    public void START_TOKEN_DOES_LEX()
    {
        var sourceText = $"{LexSolutionFacts.GlobalSection.START_TOKEN}({LexSolutionFacts.GlobalSectionNestedProjects.START_TOKEN})";
    ...
    public void FULL_DOES_LEX()
    {
        ...
        Assert.Equal(TestDataGlobalSectionNestedProjects.FIRST_PROPERTY_NAME, firstPropertyNameAssociatedNameToken.TextSpan.GetText());

        var firstPropertyValueAssociatedValueToken = (AssociatedValueToken)lexer.SyntaxTokens[i++];
        Assert.Equal(TestDataGlobalSectionNestedProjects.FIRST_PROPERTY_VALUE, firstPropertyValueAssociatedValueToken.TextSpan.GetText());

        var secondPropertyNameAssociatedNameToken = (AssociatedNameToken)lexer.SyntaxTokens[i++];
        Assert.Equal(TestDataGlobalSectionNestedProjects.SECOND_PROPERTY_NAME, secondPropertyNameAssociatedNameToken.TextSpan.GetText());

        var secondPropertyValueAssociatedValueToken = (AssociatedValueToken)lexer.SyntaxTokens[i++];
        Assert.Equal(TestDataGlobalSectionNestedProjects.SECOND_PROPERTY_VALUE, secondPropertyValueAssociatedValueToken.TextSpan.GetText());
